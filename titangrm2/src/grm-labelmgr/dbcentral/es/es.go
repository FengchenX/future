...
func (es *ESUtil) TypeQuery(index, typeName string, dataids []string, qi types.SearchInfo) (int64, []*types.MetaInfo, error) {
	...
	//			var datameta types.TypeMeta
	//			err := json.Unmarshal(*hit.Source, &datameta)
	//			if err != nil {
	//				log.Warn("TypeQuery.json.Unmarshal.source error:", err)
	//				continue
	//			}
	//
	//			datameta.DataId = hit.Id
	//			datametas = append(datametas, es.TypeMeta2MetaInfo(datameta))
	//		}
	//	}
	return searchResult.Hits.TotalHits, datametas, nil
}

func (es *ESUtil) QueryByDataset(dataids []string, key, typeName, index, order, limit, sort, offset string) (int64, []*types.MetaInfo, error) {
	var datametas []*types.MetaInfo = make([]*types.MetaInfo, 0)
