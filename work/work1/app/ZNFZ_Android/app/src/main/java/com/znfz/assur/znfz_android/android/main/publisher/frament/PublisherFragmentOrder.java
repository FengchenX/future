 ...
 */
public class PublisherFragmentOrder extends BaseFragment {


    @BindView(R.id.ll_empty_view)               // 无数据视图
    LinearLayout llEmptyView;
    @BindView(R.id.publisher_btn_order_today)   // 今日订单按钮
    Button btnTodayOrdre;
    @BindView(R.id.publisher_btn_order_history) // 历史订单按钮
    Button btnHistoryOrder;
    @BindView(R.id.publisher_btn_add_order)     // 添加订单按钮
    Button btnAddOrder;
    @BindView(R.id.publisher_listview_order)    // 订单列表
    ...
    protected int setView() {
        return R.layout.frement_publisher_order;
    }
    ...
    protected void init(View view) {
        ...
        // 初始化下啦刷新
        initRefreshLayout(new RefreshLayoutCallBack() {
            @Override
            public void onRefreshListenerBegin() {
                // 获取当前排班请求
    ...
    public void onClick(View v) {
        super.onClick(v);
        switch (v.getId()) {
            case R.id.publisher_btn_order_today:  // 今日订单
            ...
            case R.id.publisher_btn_order_history:  // 历史订单
            ...
            case R.id.publisher_btn_add_order:

                if(UserInfo.currentSchedule == null) {
                    hintDialog("当前没有排班，不能添加订单");
                    return;
                }
...
//    }
    ...
    private void getAllScheduleRequest() {
        ...
        requestBuilder.setTimeStamp(TimeUtils.getTimeSp("2018-05-19 9:10:00") + "");
                ...
                if(result.getStatusCode() == protocol.StatusOuterClass.Status.Success.ordinal()) {
                   //hintDialog(result.toString());
                    ...
                    for (int i = 0; i < orderList.size(); i++) {
                        Obj.Order order = orderList.get(i);
