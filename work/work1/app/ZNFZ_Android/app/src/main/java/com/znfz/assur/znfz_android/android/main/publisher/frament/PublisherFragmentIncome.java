...
public class PublisherFragmentIncome extends BaseFragment {



    @BindView(R.id.ll_empty_view)                       // 无数据视图
    LinearLayout llEmptyView;
    @BindView(R.id.publisher_income_detail_listview)    // 收入明细列表
    RecyclerView incomeDetailRecyclerView;

    private List<PublisherIncomeBean> listItems;         // 收入列表数据
    private PublishIncomeAdapter publishIncomeAdapter;   // 收入列表适配器


    @Override
    protected int setView() {
        return R.layout.frement_publisher_income;
    }
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        publishIncomeAdapter = new PublishIncomeAdapter(getActivity());
        incomeDetailRecyclerView.setAdapter(publishIncomeAdapter);
        //initListItemsData();
...
//        showListData();
    ...
    private void getAllScheduleRequest() {
        ...
        task.setUpdateUIInterface(new GetAllIncomeTask.UpdateUIInterface() {
            ...
            public void onSucceed(protocol.Api.RespGetAllIncome result) {
                Logger.d("GetAllIncomeTask更新UI：" + result);
                hideLoading();
    ...
    private void showListData() {
        ...
        publishIncomeAdapter.setData(listItems);
    }
}
