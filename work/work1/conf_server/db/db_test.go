...
func TestInitDb(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		//DbClient.Client.CreateTable(&OrderSave{})
		//DbClient.Client.CreateTable(&Bill{})
		...
		//err := DbClient.Client.Create(&Company{
		//	CompanyName:      "麦当基",
		//	CompanyBanchName: "南山分店",
		//	CompanyDespcrite: huangDesp,
		//	CompanyAddress:   huangAddr,
		//	CompanyPassWord:  huangPass,
		//	CompanyPhone:     "6666",
		//	CreateTime:       time.Now(),
		//})

		err := DbClient.Client.Create(&Binder{
			CompanyId:      2,
			CompanyName:    "麦当基",
			BrenchName:     "南山分店",
			UserAddress:    huangAddr,
			CompanyAddress: huangAddr,
			BindTime:       time.Now(),
		})
		if err != nil {
